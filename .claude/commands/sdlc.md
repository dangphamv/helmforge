...
3. **`/sdlc:brd`** — business-analyst → `<US-ID>-spec.md` + `requirements.md` + `acceptance.feature` + `openapi.yaml` + schema diff, and updates the living BRD (global IDs). Destructive migration → expand-and-contract plan. Opens the spec PR/MR.
   ...
   - *Optional (recommended for `standard`/`large`):* run **`/sdlc:analyze`** after `/sdlc:brd` (+ `/sdlc:design` if UI) and before **`/sdlc:build`** — a read-only coverage/consistency gate (every requirement → acceptance → task; SoTs don't contradict; constitution holds). If it reports 🔴 blocking gaps, close them via `/sdlc:brd`/`/sdlc:clarify` and re-run before building. It never edits artifacts. Skip for trivial/small.
